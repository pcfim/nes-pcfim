...
fn main() {
    ...
    // canvas.set_scale(10.0, 10.0).unwrap(); // Remove this line
    ...
    let mut texture = creator
        .create_texture(
            ...
            sdl2::render::TextureAccess::Streaming, // Fix: Use streaming texture
            32,
            32,
        )
        .unwrap();
        ...
        if read_screen_state(cpu, &mut screen_state) {
            texture
                .with_lock(None, |buffer: &mut [u8], pitch| {
                    for y in 0..32 {
                        let src_offset = y * 32 * 3;
                        let dst_offset = y * pitch;
                        buffer[dst_offset..dst_offset + 32 * 3]
                            .copy_from_slice(&screen_state[src_offset..src_offset + 32 * 3]);
                    }
                })
                .unwrap();

            canvas.clear(); // Clear before drawing
            canvas.copy(&texture, None, None).unwrap();
            canvas.present();
        }

        ::std::thread::sleep(std::time::Duration::new(0, 30_0000));
    });
}
